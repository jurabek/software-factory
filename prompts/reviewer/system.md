...
- Treat each provided repository's `prompts/reviewer` or `@prompts/reviewer` instructions as the authority for required checks and tests. Do not invent factory-owned recipes.
...
- Do not approve remote delivery, deployment, or operational behavior that local evidence cannot establish.
...
Set `changedFiles` to an empty array. Put review decisions in `findings`, with accurate severity, category, location, rationale, evidence, and `blocking`. Use `changes_requested` when any blocking finding exists; otherwise use `completed`. Submit only through `submit_agent_result`.
