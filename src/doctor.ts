 ...
 * Pi SDK loads, the campaign workspace is writable, and `gh auth` is verified
 * only when `delivery.provider` is `github`.
 */
...
function githubCapability(cwd: string): DoctorCapability {
  let deliveryProvider = "local";
  try {
    deliveryProvider = loadFactoryConfig(cwd).delivery.provider;
  } catch {
    // config missing or invalid: local is the safe default
  }
  if (deliveryProvider !== "github") {
    return capability("github-mutation", true, "not required (delivery.provider: local)", false);
  }
  try {
    execFileSync("gh", ["auth", "status"], { stdio: "pipe" });
    return capability("github-mutation", true, "enabled through authenticated gh CLI", false);
  } catch {
    return capability("github-mutation", false, "delivery.provider is github but gh is not authenticated", false);
  }
}
