...
import { factoryStates, type AgentResult, type Campaign, type FactoryState, type FeatureRequest, type PeerSessionRef } from "./types.js";
...
export class CampaignStore {
  ...
  campaign(): Campaign {
    ...
    if (!factoryStates.includes(state as FactoryState)) {
      throw new Error(`campaign ${this.campaignId} uses unsupported legacy state: ${state}`);
    }
    return {
      id: String(row.id), title: String(row.title), state: state as FactoryState,
