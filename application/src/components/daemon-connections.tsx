...
export function DaemonConnections({ login }: { login: string }) {
	...
	const navigate = useCallback(
		(next: WorkspaceSelection, replace = false) => {
		...
		try {
			...
			const message =
				error instanceof Error
					? error.message
					: "Could not load daemon connections.";
			if (message.startsWith("Session expired")) setSessionExpired(true);
	...
	useEffect(() => {
		if (
			!connections.length ||
			JSON.stringify(selection) === JSON.stringify(requested)
		)
			return;
		...
		const settledStart =
			(requested.daemonId &&
				daemonSettledLoadStart.current.get(requested.daemonId)) ||
			0;
		for (const [id, selectedAt] of pendingSelectionIds.current) {
			if (
				now - selectedAt > 30_000 ||
				allTasks.some((task) => task.id === id) ||
				settledStart > selectedAt
			)
				pendingSelectionIds.current.delete(id);
		}
		if (
			[requested.taskId, requested.sessionId].some(
				(id) => id && pendingSelectionIds.current.has(id),
			)
		)
			return;
	...
	const selected =
		connections.find((connection) => connection.id === selection.daemonId) ??
		null;
	...
	const selectedTask = selected
		? (selectedState?.tasks.find((task) => task.id === selectedTaskId) ?? null)
		: null;
	const rootTask =
		selected && selection.taskId
			? (selectedState?.tasks.find((task) => task.id === selection.taskId) ??
				null)
			: null;
	async function register(form: FormData) {
		...
		await loadTasks(response.connection);
		navigate({
			daemonId: response.connection.id,
			taskId: null,
			sessionId: null,
		});
	}
	function selectRecord(record: QualifiedTask) {
		pendingSelectionIds.current.set(record.id, Date.now());
		const rootId = record.parent_task_id ?? record.id;
		navigate({
			daemonId: record.daemonId,
			taskId: rootId,
			sessionId: record.id === rootId ? null : record.id,
		});
	}
	function selectTaskId(taskId: string) {
		const task = selectedState?.tasks.find((record) => record.id === taskId);
		if (task) selectRecord(task);
	}
	function openSession(sessionId: string, rootId: string) {
		pendingSelectionIds.current.set(sessionId, Date.now());
		navigate({ daemonId: selected?.id ?? null, taskId: rootId, sessionId });
	}
	function handleRemoved(parentTaskId?: string) {
		navigate({
			daemonId: selected?.id ?? null,
			taskId: parentTaskId ?? null,
			sessionId: null,
		});
	}
	const tasksByDaemon = Object.fromEntries(
		connections.map((connection) => [
			connection.id,
			taskStates[connection.id]?.tasks ?? [],
		]),
	);
	const offlineByDaemon = Object.fromEntries(
		connections.map((connection) => [
			connection.id,
			Boolean(taskStates[connection.id]?.offline),
		]),
	);
	return (
		<SidebarProvider
		...
		>
			<TaskRail
				...
				tasksByDaemon={tasksByDaemon}
				offlineByDaemon={offlineByDaemon}
				selection={selection}
				login={login}
			/>
			<SidebarInset className="min-w-0">
				...
				{!connections.length ? (
					<DaemonSetup onRegister={register} />
				) : !selected ? (
					...
					<TaskOverview
						key={`${selected.id}:${rootTask.id}`}
						daemon={selected}
						task={rootTask}
						login={login}
						offline={Boolean(selectedState?.offline)}
						onOpenSession={(sessionId) => openSession(sessionId, rootTask.id)}
						onCreated={(session) => {
							openSession(session.id, rootTask.id);
							void loadTasks(selected);
						}}
					/>
				) : selectedTask ? (
					<TaskDetail
						...
						onOpenTask={() => selectTaskId(rootTask.id)}
						onRemoved={handleRemoved}
					/>
				) : (
					<p className="text-muted-foreground m-4 text-center">
