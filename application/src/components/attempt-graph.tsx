	...
	if (!nodes.length)
		return (
			<p className="text-muted-foreground text-[0.78rem]">
				Attempts appear here when the task starts.
			</p>
		);
	return (
		<ol
			className="flex flex-col items-center py-4"
			aria-label="Task attempt graph"
		>
			{nodes.map((attempt, index) => {
				const selected = selectedId === attempt.id;
				const outline =
					statusBorder[attempt.status] ??
				...
				return (
					<li className="grid justify-items-center" key={attempt.id}>
						...
						>
							<span className="text-foreground truncate text-[0.72rem]">
								{attempt.name}
							</span>
							<small className="text-muted-foreground truncate text-[0.62rem]">
								#{attempt.attempt ?? 1} · {attempt.status}
								{attempt.superseded ? " · superseded" : ""}
							</small>
						</button>
						{index < nodes.length - 1 ? (
							<span
								className="border-input h-10 border-l border-dashed"
								aria-hidden="true"
							/>
						) : null}
					</li>
