...
export function SignInPanel() {
	...
	async function signIn(form: FormData) {
		...
		setError(
			response.status === 401
				...
				: "Sign-in is unavailable. Check application setup.",
		);
		setPending(false);
	}
	...
	return (
		<section aria-labelledby="signin-heading" className="mt-8 space-y-3">
			<h2 id="signin-heading" className="text-base font-medium">
				Sign in
			</h2>
			<p className="text-subtle">
				Use the initial-user credentials configured on this server.
			</p>
			{error ? (
