...
export type AuthenticationEnvironment = Pick<
...
export function validateEnvironment(environment: Environment): Validation {
	...
	for (const variable of deploymentKeys) {
		...
		if (!value?.trim()) {
			issues.push({
				variable,
				message: "Set this server-side deployment variable.",
			});
	...
	if (values.APPLICATION_ORIGIN) {
		...
		if (
			...
			issues.push({
				...
				message:
					"Use an HTTPS origin without credentials, path, query, or fragment. HTTP is allowed only on loopback outside production.",
			});
		}
	}
	if (
		values.INITIAL_USER_LOGIN &&
		!/^[a-zA-Z0-9._-]{1,64}$/.test(values.INITIAL_USER_LOGIN)
	) {
		issues.push({
			variable: "INITIAL_USER_LOGIN",
			message: "Use 1-64 letters, numbers, dots, underscores, or hyphens.",
		});
	}
	if (
		values.INITIAL_USER_PASSWORD &&
		values.INITIAL_USER_PASSWORD.length < 12
	) {
		issues.push({
	...
	if (values.DAEMON_ALLOWED_ORIGINS) {
		try {
			...
			issues.push({
				variable: "DAEMON_ALLOWED_ORIGINS",
				message:
					"Use comma-separated HTTP(S) origins with IP-address hosts; localhost is also allowed.",
			});
		}
	}
...
export function readDeploymentEnvironment(
	...
	if (!validation.ok) {
		throw new Error(
			`Invalid deployment variables: ${validation.issues.map((issue) => issue.variable).join(", ")}.`,
		);
	}
...
export function readAuthenticationEnvironment(
	...
	if (!validation.ok) {
		throw new Error(
			`Invalid authentication variables: ${validation.issues.map((issue) => issue.variable).join(", ")}.`,
		);
	}
	return validation.values;
}
