...
export function createAuthentication(options: AuthenticationOptions) {
	...
	const delayAfterFailure =
		options.delayAfterFailure ??
		(() =>
			new Promise((resolve) =>
				setTimeout(resolve, failedLoginDelayMilliseconds),
			));
	return {
		async login(login: string, password: string): Promise<string | null> {
			if (
				!matchesSecret(login, options.login) ||
				!matchesSecret(password, options.password)
			) {
		...
		async current(
			...
			const valid = await options.store.hasSession(
				digest(token).toString("hex"),
				now(),
			);
		...
		async logout(token: string | undefined): Promise<void> {
			if (token)
				await options.store.deleteSession(digest(token).toString("hex"));
		},
	};
}
...
export function createAuthStore(pool: Pool): AuthStore {
	return {
		async createSession(tokenDigest, expiresAt) {
			await pool.query(
				"INSERT INTO owner_session (token_digest, expires_at, created_at) VALUES ($1, $2, $3)",
				[tokenDigest, expiresAt.toISOString(), new Date().toISOString()],
			);
		},
		async hasSession(tokenDigest, now) {
			const result = await pool.query(
				"SELECT 1 FROM owner_session WHERE token_digest = $1 AND expires_at > $2",
				[tokenDigest, now.toISOString()],
			);
			return result.rowCount === 1;
		},
		async deleteSession(tokenDigest) {
			await pool.query("DELETE FROM owner_session WHERE token_digest = $1", [
				tokenDigest,
			]);
		},
		async deleteExpiredSessions(now) {
			await pool.query("DELETE FROM owner_session WHERE expires_at <= $1", [
				now.toISOString(),
			]);
		},
	};
