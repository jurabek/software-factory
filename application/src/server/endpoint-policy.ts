...
function parseOrigin(value: string): string {
	...
	if (
		...
		(url.protocol === "http:" &&
			hostname !== "localhost" &&
			!privateAddresses.check(hostname, family === 4 ? "ipv4" : "ipv6"))
	) {
...
export function parseAllowedDaemonOrigins(value: string): string[] {
	...
	if (!origins.length || origins.some((origin) => !origin))
		throw new Error("Daemon origin allowlist is empty.");
	return [...new Set(origins.map(parseOrigin))];
}

export function normalizeDaemonEndpoint(
	value: string,
	...
	try {
		...
		throw new Error(
			"Daemon endpoint must be an HTTP(S) origin with an IP-address host.",
		);
	}
	if (!allowedOrigins.includes(origin))
		throw new Error("Daemon endpoint is not in DAEMON_ALLOWED_ORIGINS.");
	return origin;
}
