...
import {
	GET as daemonResource,
	POST as daemonResourceMutation,
} from "../app/api/daemons/[daemonId]/tasks/[taskId]/interventions/route.ts";
import {
...
test("daemon list rejects a missing session and disables caching", async () => {
	const response = await daemonList(
...
const daemonParams = { daemonId: "daemon-a" } as unknown as {
	daemonId: string;
};
const taskParams = { daemonId: "daemon-a", taskId: "task-1" } as unknown as {
	daemonId: string;
	taskId: string;
};
...
test("every daemon read rejects a missing session before registry access", async () => {
	const responses = [
		await daemonTasks(
			new Request("http://localhost:3000/api/daemons/daemon-a/tasks"),
			{ params: Promise.resolve(daemonParams) },
		),
		await creationOptions(
			new Request(
				"http://localhost:3000/api/daemons/daemon-a/creation-options",
			),
			{ params: Promise.resolve(daemonParams) },
		),
		await daemonEvents(
			new Request(
				"http://localhost:3000/api/daemons/daemon-a/tasks/task-1/events",
			),
			{ params: Promise.resolve(taskParams) },
		),
		await daemonStream(
			new Request(
				"http://localhost:3000/api/daemons/daemon-a/tasks/task-1/events/stream",
			),
			{ params: Promise.resolve({ ...taskParams }) },
		),
		await daemonResource(
			new Request(
				"http://localhost:3000/api/daemons/daemon-a/tasks/task-1/interventions",
			),
			{ params: Promise.resolve({ ...taskParams }) },
		),
		await daemonTaskDetail(
...
test("unknown daemon commands fail without daemon access", async () => {
	...
	const response = await daemonCommand(
		new Request("http://localhost:3000/api/daemons/daemon-a/tasks/task-1/fly", {
			method: "POST",
...
test("malformed event cursors fail without daemon access", async () => {
	const response = await daemonEvents(
		new Request(
			"http://localhost:3000/api/daemons/daemon-a/tasks/task-1/events?after=not-a-number",
		),
		{ params: Promise.resolve(taskParams) },
	);
