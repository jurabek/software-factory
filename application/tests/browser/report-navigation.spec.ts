...
function startMockDaemon(): Promise<Server> {
	const mock = createServer((request, response) => {
		...
		if (path === `/api/v1/tasks/${taskID}/results`)
...
test("report navigation renders Markdown safely", async ({ page }) => {
	await login(page);
	...
	try {
		...
		await test.step("open report", async () => {
			...
			await page.getByRole("button", { name: /planner result/i }).click();
		});
		const reportView = page.locator("article", {
			hasText: "readable evidence",
		});
		await test.step("assert safe report", async () => {
			await expect(reportView).toContainText("attempt 1");
