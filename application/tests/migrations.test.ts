...
test("schema is a single portable file with both tables", async () => {
	...
	assert.match(sql, /credential TEXT NOT NULL/);
});
...
test("ensureSchema executes every statement and is idempotent", async () => {
	...
	assert.ok(
		db.queries.some((sql) =>
			sql.includes("CREATE TABLE IF NOT EXISTS owner_session"),
		),
	);
	assert.ok(
		db.queries.some((sql) =>
			sql.includes("CREATE TABLE IF NOT EXISTS daemon_connection"),
		),
	);
