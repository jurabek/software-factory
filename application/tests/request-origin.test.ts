...
test("mutation origin must exactly match the configured application origin", () => {
	assert.equal(
		hasTrustedOrigin(
			new Request("https://factory.example.com/api/login", {
				headers: { Origin: "https://factory.example.com" },
			}),
			environment,
		),
		true,
	...
	for (const origin of [
		...
		"https://factory.example.com.evil.test",
		"http://factory.example.com",
	]) {
		const headers = origin ? { Origin: origin } : undefined;
		assert.equal(
			hasTrustedOrigin(
				new Request("https://factory.example.com/api/login", { headers }),
				environment,
			),
			false,
		);
