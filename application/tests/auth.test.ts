...
function memoryStore() {
	...
	const store: AuthStore = {
		async createSession(digest, expiresAt) {
			sessions.set(digest, expiresAt);
		},
		async hasSession(digest, now) {
			return (sessions.get(digest)?.getTime() ?? 0) > now.getTime();
		},
...
test("valid configured credentials create a hashed database session", async () => {
	...
	assert.equal(
		database.sessions.get(digest)?.toISOString(),
		new Date(now.getTime() + sessionLifetimeSeconds * 1000).toISOString(),
	);
...
test("session cookie parsing ignores unrelated and empty cookies", () => {
	assert.equal(
		sessionTokenFromCookie("theme=dark; factory_session=abc123; other=value"),
		"abc123",
	);
