...
test("accepts complete deployment configuration without connecting to anything", () => {
	assert.deepEqual(validateEnvironment(configured), {
		ok: true,
		values: Object.fromEntries(
			Object.entries(configured).filter(([key]) => key !== "NODE_ENV"),
		),
	});
});

test("daemon configuration does not block otherwise valid authentication", () => {
	const { DAEMON_ALLOWED_ORIGINS: _origins, ...withoutDaemon } = configured;
...
test("missing, blank, and whitespace-padded required values fail closed", () => {
	...
	for (const key of keys) {
		for (const value of [undefined, "", " \t", " padded "]) {
			...
			if (!result.ok)
				assert.ok(result.issues.some((issue) => issue.variable === key));
		}
	}
...
for (const origin of [
	"invalid",
	"ftp://factory.example.com",
	"https://user:secret@factory.example.com",
	"https://factory.example.com/path",
	"https://factory.example.com/path/..",
	"https://factory.example.com?token=secret",
	"https://factory.example.com#secret",
	"https://factory.\nexample.com",
	...
	test(`rejects unsafe production origin: ${origin}`, () => {
		assert.equal(
			validateEnvironment({ ...configured, APPLICATION_ORIGIN: origin }).ok,
			false,
		);
	});
}
...
test("development HTTP is restricted to loopback", () => {
	for (const host of ["localhost", "127.0.0.1", "[::1]"]) {
		assert.equal(
			validateEnvironment({
				...
				APPLICATION_ORIGIN: `http://${host}:3000`,
			}).ok,
			true,
		);
	}
	assert.equal(
		validateEnvironment({
			...configured,
			NODE_ENV: "development",
			APPLICATION_ORIGIN: "http://192.168.1.2:3000",
		}).ok,
		false,
	);
});
...
test("database URLs require PostgreSQL, a host, and database", () => {
	...
	for (const value of [
		...
		"postgresql://user:p%40ss@[::1]:5432/app?sslmode=verify-full",
	]) {
...
test("login characters and password length boundary are enforced", () => {
	for (const login of [
		...
		assert.equal(
			validateEnvironment({ ...configured, INITIAL_USER_LOGIN: login }).ok,
			false,
		);
	}
	assert.equal(
		validateEnvironment({
			...configured,
			INITIAL_USER_PASSWORD: "x".repeat(11),
		}).ok,
		false,
	);
	assert.equal(
		validateEnvironment({
			...configured,
			INITIAL_USER_PASSWORD: "x".repeat(12),
		}).ok,
		true,
	);
});
...
test("validation issues never include supplied secrets or logins", () => {
	const result = validateEnvironment({
		...configured,
		DATABASE_URL: "https://user:PRIVATE_PASSWORD@db.example/app",
		APPLICATION_ORIGIN: "https://user:PRIVATE_TOKEN@factory.example",
		INITIAL_USER_LOGIN: "PRIVATE LOGIN",
		INITIAL_USER_PASSWORD: "PRIVATE",
	});
...
test("importing validation needs no deployment configuration", async () => {
	...
	const result = spawnSync(
		...
		[
			...
			"--input-type=module",
			"-e",
			`await import(${JSON.stringify(new URL("../src/server/environment.ts", import.meta.url).href)})`,
		],
		{ env: {} as NodeJS.ProcessEnv, encoding: "utf8" },
	);
