...
func TestRunTurnCorrelatesStreamedEventsToNativeEntries(t *testing.T) {
	...
	events, err := db.EventsByRequest(context.Background(), task.ID, "req-test")
	if err != nil {
		t.Fatal(err)
	}
	if len(events) != 3 {
...
func TestReconcilePendingTurnsAdoptsNativeStatsAndRequeuesPhase(t *testing.T) {
	...
	if _, err := db.ReserveAgentSession(ctx, task.ID, store.AgentSession{StageID: "builder", AgentName: "builder", Role: "builder", Harness: "pi", HarnessSessionID: "sess-1", SessionDirectory: filepath.Join(task.WorkspacePath, "sessions", "builder", "pi")}); err != nil {
		t.Fatal(err)
	}
	if err := db.BeginAgentInvocation(ctx, task.ID, "builder", "inv-1", "req-1", phase.ID); err != nil {
		t.Fatal(err)
	}
	...
	archived, err := db.PhaseByID(ctx, task.ID, phase.ID)
	if err != nil {
		t.Fatal(err)
	}
...
func TestRunTurnReusesRecoveredNativeResponseWithoutPrompting(t *testing.T) {
	...
	if err := db.BeginAgentInvocation(ctx, task.ID, "builder", "inv-1", "req-1", "phase-1"); err != nil {
		t.Fatal(err)
	}
