...
func (r *ProcessRepository) Start(ctx context.Context, taskID, phaseID,
	...
	result, err := r.db.ExecContext(ctx, `insert into processes(task_id,phase_id,kind,name,pid,display_command,status,started_at) values(?,?,?,?,?,?,?,?)`,
...
func (r *ProcessRepository) End(
	...
	_, err := r.db.ExecContext(ctx, `update processes set status=case when ?=0 then 'ended' else 'failed' end,exit_code=?,ended_at=? where task_id=? and pid=? and status='running'`,
