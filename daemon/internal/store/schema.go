...
create table if not exists tasks (
 id text primary key, parent_task_id text references tasks(id) on delete cascade,
  request text not null, workspace_path text not null,
  repository_type text not null default '', repository_source text not null default '', submitted_repository_path text,
  canonical_repository_path text, repository_path text, base_sha text, review_base_sha text, branch_name text,
...
func incompatibleSchema(ctx context.Context, db *sql.DB) (bool, error) {
	...
	for _, column := range []string{"repository_type", "repository_source", "repository_path", "base_sha", "review_base_sha", "branch_name"} {
		exists, columnErr := columnExists(ctx, db, "tasks", column)
	...
	for _, column := range []string{"kind", "format_version", "display_json", "native_entry_id"} {
		exists, columnErr := columnExists(ctx, db, "events", column)
		if columnErr != nil || !exists {
			return true, columnErr
		}
	}
	// Native sessions are authoritative in this format; the previous
	// per-invocation accounting model is gone. Refuse state that predates it.
	legacyAccounting, err := columnExists(ctx, db, "agent_sessions", "accounting_complete")
	if err != nil || legacyAccounting {
		return true, err
	}
	for _, column := range []string{"harness_session_id", "session_ready", "native_transcript_path", "pending_invocation_id", "pending_request_id", "pending_phase_id", "last_entry_id"} {
