...
func (r *EvidenceRepository) SaveTestChanges(ctx context.
...
func (r *EvidenceRepository) TestChanges(ctx context.Context, taskID string) ([]TestChange,

	error,
) {
	rows, err := r.db.QueryContext(ctx, `select id,task_id,phase_id,attempt,path,reason,change_kind,coalesce(rename_from,''),coalesce(rename_to,''),created_at from test_changes where task_id=? order by created_at,rowid`,
...
func (r *EvidenceRepository) SaveComparison(ctx context.
	...
	_, err = r.db.ExecContext(ctx, `insert or replace into comparisons(id,task_id,phase_id,attempt,status,reason,baseline_snapshot,overlay_paths_json,created_at,duration_ms) values(?,?,?,?,?,?,?,?,?,?)`,
...
func (r *EvidenceRepository) Comparisons(ctx context.Context, taskID string) ([]Comparison,

	error,
) {
	rows, err := r.db.QueryContext(ctx, `select id,task_id,phase_id,attempt,status,reason,coalesce(baseline_snapshot,''),overlay_paths_json,created_at,duration_ms from comparisons where task_id=? order by created_at,rowid`,
