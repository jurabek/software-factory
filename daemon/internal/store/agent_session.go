...
func (db *DB) BeginAgentInvocation(ctx context.Context, taskID, role, invocationID, requestID, phaseID string) error {
	result, err := db.ExecContext(ctx, `update agent_sessions set pending_invocation_id=?,pending_request_id=?,pending_phase_id=?,last_used_at=? where task_id=? and stage_id=? and pending_invocation_id is null`, invocationID, nullIfEmpty(requestID), nullIfEmpty(phaseID), now(), taskID, role)
	if err != nil {
...
func (db *DB) PendingAgentSessions(ctx context.Context) ([]AgentSession, error) {
	...
	if err != nil {
		return nil, wrap("list pending agent sessions", err)
	}
	defer rows.Close()
	type key struct{ taskID, stageID string }
	keys := make([]key, 0)
	for rows.Next() {
		var value key
	...
	for _, value := range keys {
		session, readErr := db.AgentSession(ctx, value.taskID, value.stageID)
		if readErr != nil {
			return nil, readErr
		}
...
func (db *DB) ReconcileAgentStats(ctx context.Context, taskID, stageID string, value AgentSession) error {
	...
	tx, err := db.BeginTx(ctx, nil)
	if err != nil {
		return wrap("begin agent reconcile", err)
	}
	defer tx.Rollback()
	var previousCost float64
	if err = tx.QueryRowContext(ctx, `select coalesce(cost,0) from agent_sessions where task_id=? and stage_id=?`, taskID, stageID).Scan(&previousCost); err != nil {
		return wrap("read agent session cost", err)
	}
	if _, err = tx.ExecContext(ctx, `update agent_sessions set provider=?,model=?,native_transcript_path=?,last_entry_id=?,context_tokens=?,context_window=?,usage_json=?,cost=?,last_used_at=? where task_id=? and stage_id=?`, nullIfEmpty(value.Provider), nullIfEmpty(value.Model), nullIfEmpty(value.NativeTranscriptPath), nullIfEmpty(value.LastEntryID), value.ContextTokens, value.ContextWindow, string(usage), value.Cost, now(), taskID, stageID); err != nil {
		return wrap("reconcile agent session", err)
	}
	if delta := value.Cost - previousCost; delta != 0 {
		if _, err = tx.ExecContext(ctx, `update tasks set total_cost=total_cost+? where id=?`, delta, taskID); err != nil {
			return wrap("update task agent cost", err)
...
func (db *DB) ClearAgentInvocation(ctx context.Context, taskID, stageID, invocationID string) error {
	result, err := db.ExecContext(ctx, `update agent_sessions set pending_invocation_id=null,pending_request_id=null,pending_phase_id=null,last_used_at=? where task_id=? and stage_id=? and pending_invocation_id=?`, now(), taskID, stageID, invocationID)
	if err != nil {
...
func (db *DB) RequeueInterruptedPhase(ctx context.Context, taskID, phaseID string) error {
	_, err := db.ExecContext(ctx, `update phases set status='queued',error=null,ended_at=null where task_id=? and id=? and status='interrupted'`, taskID, phaseID)
...
func (db *DB) ResetAgentSession(ctx context.Context, taskID, stageID, newSessionID string) error {
	result, err := db.ExecContext(ctx, `update agent_sessions set harness_session_id=?,session_ready=0,native_transcript_path=null,last_entry_id=null,pending_invocation_id=null,pending_request_id=null,pending_phase_id=null,last_used_at=? where task_id=? and stage_id=? and pending_invocation_id is null`, newSessionID, now(), taskID, stageID)
	if err != nil {
		return wrap("reset agent session", err)
	}
	count, err := result.RowsAffected()
	if err != nil {
		return wrap("read agent session reset", err)
	}
	if count == 0 {
		var exists int
		if readErr := db.QueryRowContext(ctx, `select count(*) from agent_sessions where task_id=? and stage_id=?`, taskID, stageID).Scan(&exists); readErr != nil {
			return wrap("read agent session for reset", readErr)
		}
		if exists == 0 {
			return ErrNotFound
		}
...
func (db *DB) ReplaceAgentSession(ctx context.Context, taskID, role, newSessionID, newDirectory string) (priorID string, err error) {
	...
	result, err := db.ExecContext(ctx, `update agent_sessions set harness_session_id=?,session_directory=?,session_ready=0,native_transcript_path=null,last_entry_id=null,last_used_at=? where task_id=? and stage_id=? and harness_session_id=? and pending_invocation_id is null`, newSessionID, newDirectory, now(), taskID, role, current.HarnessSessionID)
	if err != nil {
