...
// publishBuild delivers message turns, enforces protected paths, persists test
...
func (s service) publishBuild(ctx context.Context, task store.Task, phase store.Phase, turn harness.TurnResult, profile workspace.Materialization) (stage.BuildResult, error) {
	return s.kit.DeliverAndFinalize(ctx, stagekit.Delivery{
		Task: task, Phase: phase, Role: "build", ReadOnly: readOnly(phase), Instructions: Instructions(),
		...
		OnValid: func(ctx context.Context, text string) error {
			return PersistEvidence(ctx, s.kit.DB(), task, phase, text)
		},
...
func (s service) validatePaths(task store.Task, profile workspace.Materialization) error {
	return CheckProtectedPaths(task.RepositoryPath, workspace.ReviewBase(task), profile.Protected)
}
